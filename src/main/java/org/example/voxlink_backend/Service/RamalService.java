...
public class RamalService {
    ...
    public String desassociarUsuarioDoRamal(String codigoLogin) {
        Optional<Ramal> ramalOpt = ramalRepository.findByUsuario_CodigoLogin(codigoLogin);
        if (ramalOpt.isEmpty()) {
            return "Usuário não está associado a nenhum ramal.";
        }

        Ramal ramal = ramalOpt.get();
        ramal.setUsuario(null); // Remove a associação
        ramalRepository.save(ramal);

        return "Usuário desassociado do ramal com sucesso.";
    }
