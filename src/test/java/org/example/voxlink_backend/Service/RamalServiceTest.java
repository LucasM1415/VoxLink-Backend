...
class RamalServiceTest {
    ...
    void desassociarUsuarioDoRamal_Success() {
        ...
        // Chamando o método
        String resultado = ramalService.desassociarUsuarioDoRamal("USER001");

        // Verificando se o resultado é o esperado
        assertEquals("Usuário desassociado do ramal com sucesso.", resultado);
    ...
    @Test
    void desassociarUsuarioDoRamal_UsuarioNaoAssociado() {
        // Simulando que o ramal não tem usuário associado
        when(ramalRepository.findByUsuario_CodigoLogin("USER001")).thenReturn(Optional.empty());

        // Chamando o método
        String resultado = ramalService.desassociarUsuarioDoRamal("USER001");

        // Verificando se a mensagem correta foi retornada
        assertEquals("Usuário não está associado a nenhum ramal.", resultado);
    }
}
